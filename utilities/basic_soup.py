...
def get_single_page_html(url_to_scrape):
    ...
    with requests.Session() as session:
        ...
        try:
            #logger.info('Start harvesting operation for URL: ' + url_to_scrape)
            raw_html = session.get(url_to_scrape, headers=http_headers, allow_redirects=True, verify=True, timeout=30)

        except requests.HTTPError as e:
            print (e)
            #logger.error(f'A HTTP error has occurred. Please verify that the following Domain/URL is valid: {url_to_scrape}')
            # Logs a message with level ERROR and dumps a stack trace
            #logger.exception('')
            # Force exit after the error was logged
            sys.exit(1)
            ...
            print (e)
            #logger.error(f'Failed to establish a connection to: {url_to_scrape}')
            # Logs a message with level ERROR and dumps a stack trace
            #logger.exception('')
            # Force exit after the error was logged
            sys.exit(1)
            ...
            #logger.error(f'A connection timeout has occurred. Please check the internet connection and/or verify that the following Domain/URL is valid: {url_to_scrape}')
            # Logs a message with level ERROR and dumps a stack trace
            #logger.exception('')
            # Force exit after the error was logged
            sys.exit(1)
            ...
            print (e)
            #logger.error(f'An ambiguous exception occurred while handling the harvest request to the following URL: {url_to_scrape}')
            # Logs a message with level ERROR and dumps a stack trace
            #logger.exception('')
            # Force exit after the error was logged
            sys.exit(1)
...
def query_html(raw_html, tag_type, tag_attribute, attribute_text):
    ...
    :return: The section of the HTML containing the advisory information that requires further parsing
    """
    advisorySoupParser = BeautifulSoup(raw_html, 'lxml')

    try:
       results = advisorySoupParser.find(tag_type, {tag_attribute: attribute_text})

    except AttributeError as e:
       #logging.error('An unknown error occurred while trying to read the table using BeautifulSoup.')
       # Logs a message with level ERROR and dumps a stack trace
       print (e)
       # Force exit after the error was logged
       sys.exit(1)
